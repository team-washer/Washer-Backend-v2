...
public record PenaltyStatusDto(@Schema(description = "사용자 ID", example = "1") Long userId,
    ...
    public static PenaltyStatusDto of(Long userId, boolean isPenalized, LocalDateTime penaltyExpiresAt) {
        Long remainingMinutes = null;
        if (isPenalized && penaltyExpiresAt != null) {
            remainingMinutes = java.time.Duration.between(LocalDateTime.now(), penaltyExpiresAt).toMinutes();
        }
        return new PenaltyStatusDto(userId, isPenalized, penaltyExpiresAt, remainingMinutes);
    }
}
