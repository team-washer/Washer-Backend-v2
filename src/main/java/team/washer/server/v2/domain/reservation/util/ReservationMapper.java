 ...
 *
 * <p>
 * 정적 팩토리 메서드를 통해 Reservation 엔티티를 ReservationResDto로 변환한다. 이 클래스는 인스턴스화될 수 없으며, 모든 메서드는 정적 메서드로
 * 제공된다.
 * </p>
 */
public final class ReservationMapper {
    ...
    public static ReservationResDto toResDto(final Reservation reservation) {
        return new ReservationResDto(reservation.getId(), reservation.getUser().getId(),
                reservation.getUser().getName(), reservation.getUser().getRoomNumber(),
                reservation.getMachine().getId(), reservation.getMachine().getName(),
                reservation.getReservedAt(), reservation.getStartTime(), reservation.getExpectedCompletionTime(),
