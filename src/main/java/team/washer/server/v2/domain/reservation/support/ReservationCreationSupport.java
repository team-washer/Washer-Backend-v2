 ...
 * {@code lockReservationPolicyScope} → {@code validateRoomConstraints} →
 * {@code lockMachine} → {@code validateMachineAndReservations} →
 * {@code create}로 저장 직전 불변식을 다시 확인한다. 특히 기기 비관적 락이 중복 검증보다 먼저 수행되어야 동일 기기에 대한
 * 동시 예약이 직렬화된다.
 */
@Component
