...
import team.washer.server.v2.domain.reservation.service.ReservationService;

// TODO: 인증 시스템 구현 후 수정 필요
// 1. 모든 메서드의 @RequestParam Long userId 제거
// 2. @AuthenticationPrincipal 또는 SecurityContext에서 userId 추출
...
public class ReservationController {
    ...
    public ReservationResponseDto createReservation(
            ...
            @Parameter(description = "예약 생성 요청 DTO") @RequestBody @Valid CreateReservationRequestDto requestDto) {
    ...
    public ReservationResponseDto startReservation(
            @Parameter(description = "사용자 ID (임시: 인증 시스템 구현 후 제거 예정)", required = true) @RequestParam @NotNull Long userId,
            @Parameter(description = "예약 ID") @PathVariable @NotNull Long id,
            @Parameter(description = "시작 요청 DTO") @RequestBody @Valid StartReservationRequestDto requestDto) {
        return reservationService.startReservation(userId, id, requestDto);
    }

    @DeleteMapping("/{id}")
    @Operation(summary = "예약 취소", description = "예약을 취소합니다. RESERVED 상태에서 취소 시 10분간 예약이 제한됩니다.")
    public CancellationResponseDto cancelReservation(
            @Parameter(description = "사용자 ID (임시: 인증 시스템 구현 후 제거 예정)", required = true) @RequestParam @NotNull Long userId,
            @Parameter(description = "예약 ID") @PathVariable @NotNull Long id) {
    ...
    public ReservationResponseDto getActiveReservation(
            @Parameter(description = "사용자 ID (임시: 인증 시스템 구현 후 제거 예정)", required = true) @RequestParam @NotNull Long userId) {
    ...
    public Page<ReservationHistoryDto> getReservationHistory(
            @Parameter(description = "사용자 ID (임시: 인증 시스템 구현 후 제거 예정)", required = true) @RequestParam @NotNull Long userId,
        ...
        return reservationService.getReservationHistory(userId, status, startDate, endDate, machineType, pageable);
    }

    @GetMapping("/{id}")
    @Operation(summary = "예약 상세 조회", description = "예약 상세 정보를 조회합니다.")
    public ReservationResponseDto getReservation(@Parameter(description = "예약 ID") @PathVariable @NotNull Long id) {
