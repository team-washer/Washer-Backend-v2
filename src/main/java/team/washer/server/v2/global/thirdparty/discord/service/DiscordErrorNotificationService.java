...
public class DiscordErrorNotificationService {
    ...
    private DiscordEmbed createErrorEmbed(Throwable exception, String context, Map<String, Object> additionalInfo) {
        ...
        if (!stackTrace.isEmpty()) {
            fields.add(DiscordField.builder().name("Stack Trace").value("```" + stackTrace.toString() + "```")
                    .inline(false).build());
        }
        return DiscordEmbed.builder().title("🚨 애플리케이션 에러 발생").color(EmbedColor.ERROR.getColor()).fields(fields)
