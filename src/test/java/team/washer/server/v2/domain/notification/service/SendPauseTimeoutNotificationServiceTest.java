...
import team.washer.server.v2.domain.notification.service.impl.SendPauseTimeoutNotificationServiceImpl;
import team.washer.server.v2.domain.user.entity.User;

@ExtendWith(MockitoExtension.class)
class SendPauseTimeoutNotificationServiceTest {

    @InjectMocks
    private SendPauseTimeoutNotificationServiceImpl sendPauseTimeoutNotificationService;
    ...
    @Nested
    @DisplayName("일시정지 초과 알림 전송")
    class ExecuteTest {

        @Test
        @DisplayName("일시정지 초과 알림을 DB에 저장하고 FCM으로 전송한다")
        void execute_ShouldSaveNotificationAndSendFcm_WhenValidUserAndMachine() {
            ...
            // When
            sendPauseTimeoutNotificationService.execute(user, machine);
        ...
        @Test
        @DisplayName("건조기 일시정지 초과 알림을 DB에 저장하고 FCM으로 전송한다")
        void execute_ShouldSaveNotificationAndSendFcm_WhenDryer() {
            // Given
            when(machine.getName()).thenReturn("D-3F-L1");
